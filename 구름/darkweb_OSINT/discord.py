...
TOR_PATH = r"C:\Users\Laiika\Desktop\Tor Browser\Browser\Tor\tor.exe"
...
def save_to_mongodb(data_list):
    try:
        ...
        # Trigger the print_alert event
        loop = asyncio.get_event_loop()
        loop.create_task(bot.print_alert(data_list))

    except Exception as e:
        logging.error(f"Error connecting to MongoDB: {e}")

# Constants for your Discord Bot
TOKEN = ''# 깃허브 경고로 토큰을 삭제한 상태
CHANNEL_ID = '1245272086834843708'

def format_date(date_str):
    return datetime.strptime(date_str, '%y-%m-%d').strftime('%Y-%m-%d')

# Function to read data from MongoDB
def read_from_mongodb(date):
    client = MongoClient('mongodb://localhost:27017/')
    db = client['darkweb']
    collection = db[date]
...
# Function to read HTML content from MongoDB
def get_html_content(date):
    client = MongoClient('mongodb://localhost:27017/')
    db = client['darkweb']
    collection = db[date]
    documents = collection.find()
    html_content = [doc['html_content'] for doc in documents]
    client.close()
    return html_content


# Function to create HTML files from content
def create_html_files(html_content, output_dir):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    file_paths = []
    for idx, content in enumerate(html_content):
        file_path = os.path.join(output_dir, f'content_{idx + 1}.html')
        with open(file_path, 'w', encoding='utf-8') as file:
            file.write(content)
        file_paths.append(file_path)
    return file_paths

# Function to create a zip file of the HTML files
...
async def on_ready():
    ...
    print('------')
    print(f"Bot is running {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    channel = bot.get_channel(int(CHANNEL_ID))
    if channel:
        await channel.send(f"Bot is running {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        await channel.send("Usage:\n!getdata 'YYYY-MM-DD'\n!getzip 'YYYY-MM-DD'")
    else:
        print(f"Channel with ID {CHANNEL_ID} not found")

@bot.event
async def print_alert(data_list):
    channel = bot.get_channel(int(CHANNEL_ID))
    if channel:
        for data in data_list:
            company, country, site_urls, date_added, date_published = data
            date_added = format_date(date_added)
            date_published = format_date(date_published)
            message = (f"Breach detected!\nCompany: {company}\nCountry: {country}\nURL: {site_urls}\n"
...
async def check(ctx):
    await ctx.send('we running :)')

# Command to get data from MongoDB
@bot.command(name='getdata')
async def get_data(ctx, date: str):
    try:
        data = read_from_mongodb(date)
        if data:
            for document in data:
                message = (f"Breach detected!\nCompany: {document.get('company')}\nCountry: {document.get('country')}\nURL: {document.get('site_urls')}\n"
                           f"Date Added: {document.get('date_added')}\nPublication Date: {document.get('date_published')}")
...
# Command to generate zip of HTML files for a specified date
@bot.command(name='getzip')
async def get_zip(ctx, date: str):
    try:
        html_content = get_html_content(date)
        if html_content:
            output_dir = f'temp_html_{date}'
            file_paths = create_html_files(html_content, output_dir)
            output_zip = f'{date}_html_files.zip'
            create_zip(file_paths, output_zip)
            await ctx.send(file=discord.File(output_zip))
            os.remove(output_zip)  # Clean up the zip file after sending
            ...
            await ctx.send(f"No HTML content found for {date}")
    except Exception as e:
        await ctx.send(f"Error generating zip file: {e}")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            logging.info(f"Run BreachBot...")
            base_url = "http://mbrlkbtq5jonaqkurjwmxftytyn2ethqvbxfu4rgjbkkknndqwae6byd.onion/"  # 실제 다크웹 사이트의 URL로 대체
