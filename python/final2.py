...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(port_scan, target, port) for port in range(1, 499)]
        for future in tqdm(concurrent.futures.as_completed(futures), total=len(range(1, 499)), desc="Port Scanning", unit="port"):
